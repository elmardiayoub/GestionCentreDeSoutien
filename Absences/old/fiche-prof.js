...
xmlhttp.onreadystatechange = function() {
  ...
  if (this.readyState == 4 && this.status == 200) {
    ...
    {
       // [identif du cours] [select] [matiere ] [niveau] 
      ...
      //	var foot=document.getElementsByTagName('tfoot');
        ...
        button1.innerHTML="Identifiant_cours";
        head[0].appendChild(button1);
         var button2 = document.createElement('th');
        button2.innerHTML="Identifiant_cours";
...
//	document.getElementById('save').style.display="none";
         ...
         if(myObj[i].id_cours!=null){
         	
        case1.innerHTML=myObj[i].id_cours;

        var case2= document.createElement('td');
       	var in=document.createElement('input');
       	in.setAttribute('type','checkbox');
       	in.setAttribute("id",myObj[i].id_cours);
       	case2.appendChild(in);

 var case3 = document.createElement('td');

        case3.innerHTML=myObj[i].libellé_mat;

 var case4 = document.createElement('td');

        case4.innerHTML=myObj[i].libellé_niv;
...
if(something[0]==='prof')
    xmlhttp.open("GET", "requete/tableau_init.php?prof="+something[1], true);
